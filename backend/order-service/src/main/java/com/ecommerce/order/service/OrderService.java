...
public class OrderService {
    ...
    public OrderResponse createOrder(Long userId, OrderRequest request) {
        ...
        // SECURITY WARNING: In a production environment, prices MUST be fetched from the Product Service/Database.
        // Trusting the frontend price is a vulnerability.
        // For Phase 1 Prototype, we use the requested price, but this should be refactored to:
        ...
        kafkaTemplate.send(TOPIC_ORDER_CREATED, savedOrder.getId().toString(), event);
    ...
    private OrderResponse mapToResponse(Order order) {
        return new OrderResponse(
                ...
                order.getItems().stream()
                        .map(i -> new com.ecommerce.order.dto.OrderItemResponse(i.getProductId(), i.getQuantity(), i.getPrice()))
                        .collect(Collectors.toList()),
                order.getCreatedAt()
        );
    }
